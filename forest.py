...
class Forest(object):
    ...
    def add(self, e):
        ...
        if self.viterbi is not None:
            self.viterbi[e.head] = max(self.viterbi[e.head], (product(self.viterbi[tail][0] for tail in e.tails)*e.weight, e))
    ...
    def _compute_outside(self):
        ...
        for bi in xrange(len(buckets)-1, -1, -1):
            bucket = buckets[bi]
    ...
    def derivation_helper(self, node, rank, fun):
        ...
        tailvalues = [self.derivation_helper(tail, tailrank, fun) for (tail, tailrank) in itertools.izip(edge.tails, tailranks)]
...
class KBest(object):
    ...
    def push(self, edge, ranks=None):
        ...
        weight = edge.weight * product(self.f._kbest[tail][rank][0] for (tail,rank) in itertools.izip(edge.tails, ranks))
...
def earley(g, w):
    ...
    chart = set()
    doneindex = dict(((i,x),set()) for x in g.nodes for i in xrange(len(w)+1))
    nextindex = dict(((x,i),set()) for x in g.nodes for i in xrange(len(w)+1))
